...
### Fixed
- `.github/workflows/ci.yml`: forward `PROMOTER_SSH_KEY` to the reusable `shopping-cart-infra/.github/workflows/build-push-deploy.yml`. The publish job passed `PACKAGES_TOKEN`, `COSIGN_KEY` and `COSIGN_PASSWORD` but not the promoter key, so the promote step wrote an empty key file and died at `Load key "~/.ssh/promoter_key": error in libcrypto`. The image built, pushed and was attested correctly every time — only the git promotion of `newTag:` in `k8s/base/kustomization.yaml` failed, and because that step is `continue-on-error: true` and the publish job is skipped on pull requests, no PR could reveal it. Broken for six consecutive pushes to `main` from 2026-08-12: this repo was never onboarded in the 2026-08-09 SSH-promoter rollout, which enumerated no repo list, and a Dependabot pin bump then imported the breaking change green on 2026-08-12. Requires the `sc-image-promoter` write deploy key and the `PROMOTER_SSH_KEY` secret, both added 2026-09-21; deploy keys are per-repo and are not interchangeable between the sibling services.
